...
const buildModel = function () {
  ...
  model.add(
    tf.layers.conv2d({
      inputShape: [imageWidth, imageHeight, imageChannels], // tensors between layers automatic so only need to format input shape on 1st layers
      filters: 8,
      kernelSize: 5,
      padding: "same",
      activation: "relu",
  ...
  model.add(
    tf.layers.dense({
      units: numOfClasses,
      activation: "softmax", // classification model
    })
  );
